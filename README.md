...
## 发射与接受设备的距离以及相对位置
选择一本较厚的书，选择封面距离为1cm的页码将手机夹入，使得手机平行于书本平行于电脑如下图1所示。再通过电脑的摄像头取得的画面可以非常好的判断发光二极管是否与接收设备对齐。<br />
 ![测量准备](img/测量.png)<br> 
 ![对准](img/对准.png)<br>
 上图B即为测量时的。注意：测量准备并不开手机本身的照明LED；准备以及测量时均在黑暗的环境下进行。<br>
<table width="723" height="139" border="0">
  <tbody>
    <tr>
      <td width="156" rowspan="2"><div>
        <div><strong>无光照下的信号</strong></div>
      </div></td>
      <td width="186" rowspan="2"><div>
        <div><strong>仅660nm光照下的信号</strong></div>
      </div></td>
      <td colspan="2"><div>
        <div><strong>手指放入检测的数据</strong></div>
      </div></td>
    </tr>
    <tr>
      <td>&nbsp;</td>
